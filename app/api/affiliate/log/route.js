...
const TON_WALLET_ADDRESS = process.env.TON_WALLET_ADDRESS || 'UQBKK8o7TYYTGIm8BDTBiG2xBUvpj0tEYoXXd3SFLqJbhBhp';
...
export async function POST(request) {
  try {
    ...
    const rewardNote = eventType === 'reward' ? ' ton_wallet=' + TON_WALLET_ADDRESS : '';
    const systemLog = await logSystemEvent(eventType, 'affiliate ' + eventType + ' received' + rewardNote, referralCode || telegramUserId || source);
    ...
    return NextResponse.json({ ok: true, systemLogId: systemLog.id, affiliateLogId: affiliateLog.id, summaryId: summary.id, tonWalletAddress: TON_WALLET_ADDRESS });
  } catch (error) {
